...
$c->add_dataset_trigger( "eprint", EP_TRIGGER_RDF, sub {
	...
	DOC: foreach my $doc ( @{$eprint->get_value( "documents" )} )
	{
		...
		$label .= " (".$eprint->repository->xml->to_string( $doc->render_value( "format" ) ).")";
                ...
                foreach my $v ( @ {$eprint->get_value( "title" )})
                {
				...
				$o{graph}->add( 
					...
					predicate => "dct:hasPart",
	   			   	   object => "<$url>" );
	   			   	      ...
	   			   	      use utf8;
	   			   	      use Encode;
	   			   	       my $string=$file->get_value( "filename" );
	   			   	       #utf8::downgrade($string);
	   			   	      #utf8::encode( $string);
	   			   	       
  #print Dumper(Encode::encode_utf8($string));
  #print Dumper(utf8::is_utf8($string));

	   			   	   #foreach my $v ( @ {$file->get_value( "filename" )})
                                           #{
			                      ...
			                      predicate => "rdfs:label",
			                      #object =>$v->{name} ,
			                      object =>Encode::encode_utf8($string),
			                      type => "xsd:string"
                                               );
#                                              }
